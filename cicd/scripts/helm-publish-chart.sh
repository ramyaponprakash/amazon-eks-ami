...
HELM_ROOT=$2
[[ -z "${HELM_ROOT}" ]] && echo "HELM_ROOT is required" && exit 1
...
aws s3api head-object --bucket "$BUCKET_NAME" --key "charts/$CHART_NAME/index.yaml" > /dev/null 2>&1 || INDEX_NOT_EXIST=true
...
helm repo add "$CHART_NAME" "$BUCKET"

echo "packaging $CHART_NAME"
helm package "$HELM_ROOT/$CHART_NAME"/ --destination "$DEST_PATH" || exit 1
...
for file in "$DEST_PATH"/*; do
  ...
  if [ "$VERSION_RESULT" == "No results found" ]; then
     ...
     helm s3 push "$DEST_PATH/$FILE_NAME" "$CHART_NAME" --force || exit 1
  else
