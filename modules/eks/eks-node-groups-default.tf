...
resource "aws_launch_template" "default_nodegroup" {
  ...
  vpc_security_group_ids = [aws_security_group.eks_cluster-cluster.id, aws_security_group.eks_cluster-node.id]
  ...
  user_data = base64encode(templatefile("${path.module}/eks-node-groups-userdata.tpl",
    {
      ...
      NO_PROXY_POD   = "${join(",", data.aws_vpc.vpc.cidr_block_associations[*].cidr_block)},${aws_eks_cluster.eks_cluster.kubernetes_network_config[0].service_ipv4_cidr},localhost,127.0.0.1,169.254.169.254,.local,.svc,.internal,.eks.amazonaws.com,${var.eks_private_ep_no_proxy}${var.eks_additional_no_proxy}"
      MAX_POD        = "17"
    }
  ))
