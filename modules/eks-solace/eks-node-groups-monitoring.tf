resource "aws_eks_node_group" "monitoring" {
  ...
  subnet_ids    = [var.eks_private_subnet_ids[2]]
  ...
  user_data = base64encode(templatefile("${path.module}/eks-node-groups-userdata.tpl",
    {
      ...
      NO_PROXY_POD   = "${join(",", data.aws_vpc.vpc.cidr_block_associations[*].cidr_block)},${data.aws_eks_cluster.eks_cluster.kubernetes_network_config[0].service_ipv4_cidr},${var.eks_default_no_proxy},${var.eks_private_ep_no_proxy}${var.eks_additional_no_proxy}"
      MAX_POD        = "17"
    }
  ))
